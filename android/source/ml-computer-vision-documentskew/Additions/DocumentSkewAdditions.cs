...
namespace Huawei.Hms.Mlsdk.Dsc
{
    public partial class MLDocumentSkewCorrectionAnalyzer
    {
        //async Task<NAME> MyMethod(params) => await MyNativeMethod(params).CastTask<NAME>();
        public async Task<MLDocumentSkewCorrectionResult> DocumentSkewCorrectAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame, Huawei.Hms.Mlsdk.Dsc.MLDocumentSkewCorrectionCoordinateInput coordinateInput) =>
            await AsyncDocumentSkewCorrect(frame, coordinateInput).CastTask<MLDocumentSkewCorrectionResult>();

        //async Task<NAME> MyMethod(params) => await MyNativeMethod(params).CastTask<NAME>();
        public async Task<MLDocumentSkewDetectResult> DocumentSkewDetectAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame) =>
            await AsyncDocumentSkewDetect(frame).CastTask<MLDocumentSkewDetectResult>();
    }
}
