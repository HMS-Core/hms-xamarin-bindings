...
namespace Huawei.Hms.Mlsdk.Skeleton
{
    public partial class MLSkeletonAnalyzer
    {
        //async Task<IList<TYPE>> MyMethod(params) => await MyNativeMethod(params).CastTask<JavaList<TYPE>>();
        public async Task<IList<MLSkeleton>> AnalyseFrameAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame) =>
            await AsyncAnalyseFrame(frame).CastTask<JavaList<MLSkeleton>>();
    }
}
