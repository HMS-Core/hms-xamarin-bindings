...
namespace Huawei.Hms.Mlsdk.Scd
{
    public partial class MLSceneDetectionAnalyzer
    {
        //async Task<IList<TYPE>> MyMethod(params) => await MyNativeMethod(params).CastTask<JavaList<TYPE>>();
        public async Task<IList<MLSceneDetection>> AnalyseFrameAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame) =>
            await AsyncAnalyseFrame(frame).CastTask<JavaList<MLSceneDetection>>();
    }
}
