...
namespace Huawei.Hms.Mlsdk.Handkeypoint
{
    public partial class MLHandKeypointAnalyzer
    {
        //async Task<IList<TYPE>> MyMethod(params) => await MyNativeMethod(params).CastTask<JavaList<TYPE>>();
        public async Task<IList<MLHandKeypoints>> AnalyseFrameAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame) =>
            await AsyncAnalyseFrame(frame).CastTask<JavaList<MLHandKeypoints>>();
    }
}
