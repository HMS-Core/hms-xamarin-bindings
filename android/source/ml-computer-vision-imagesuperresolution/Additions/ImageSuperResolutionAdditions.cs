...
namespace Huawei.Hms.Mlsdk.Imagesuperresolution
{
    public partial class MLImageSuperResolutionAnalyzer
    {
        //async Task<NAME> MyMethod(params) => await MyNativeMethod(params).CastTask<NAME>();
        public async Task<MLImageSuperResolutionResult> AnalyseFrameAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame) =>
            await AsyncAnalyseFrame(frame).CastTask<MLImageSuperResolutionResult>();
    }
}
