...
namespace Huawei.Hms.Mlsdk.Fr
{
    public partial class MLFormRecognitionAnalyzer
    {
        //async Task<NAME> MyMethod(params) => await MyNativeMethod(params).CastTask<NAME>();
        public async Task<GoogleGson.JsonObject> AnalyseFrameAsync(Huawei.Hms.Mlsdk.Common.MLFrame frame) =>
            await AsyncAnalyseFrame(frame).CastTask<GoogleGson.JsonObject>();
    }
}
